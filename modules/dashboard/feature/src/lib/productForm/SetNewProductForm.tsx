...
import { UploadExternalFiles } from '@productize-v1.0.0/modules/dashboard/ui';
...
export const SetNewProductForm = () => {
  ...
  const [dataPreview, setDataPreview] = useState<File[]>([]);
  const [coverPhotoPreview, setCoverPhotoPreview] = useState<File[]>([]);
  ...
  const showFiles = (inputId: string) => {
  ...
  const urlToFile = async (url: RequestInfo | URL) => {
  ...
  const createFileList = useCallback(
    async (urls: string[]): Promise<File[]> => {
      try {
  ...
  }, [createFileList, state?.product?.cover_photos, state?.product?.data]);
  ...
  }, [setValue, state, location, dataPreview, coverPhotoPreview, hash]);
      ...
      <SimpleGrid my={8}>
        ...
        <Box mt={`5rem`} mb={8}>
          ...
          <Box>
            ...
            <SimpleGrid
              display={dataPreview?.length ? `grid` : `none`}
              my={4}
              columns={{ base: 1, md: 2 }}
              gap={4}
            >
        ...
        <Box my={8}>
          ...
          <SimpleGrid
            display={coverPhotoPreview?.length ? `grid` : `none`}
            my={4}
            columns={{ base: 1, md: 3 }}
            gap={4}
          >
            {coverPhotoPreview?.map((file: File) => {
              return (
      ...
      <Flex gap={4} alignItems={`center`}>
        ...
        <Box>
          ...
          <Text className="small-text">
            {`${file.type}.${Math.floor(file.size / 1000)}`}kb
          </Text>
        </Box>
      </Flex>
    </Card>
...
const HighLightField = ({ name }: highlightProps) => {

  ...
  const navLink1 = links1?.map((link) => {
    return (
      <NavLink state={link?.name} className={({ isActive }) => (isActive ? style.active : style.inactive)} to={link?.path} key={link?.id}>
        <Flex borderRadius={`8px`} width={`195px`} py={`12px`} px={`16px`} fontSize={`1.5rem`} alignItems={`center`} justifyContent={`space-between`} w={`100%`}>
          <Flex w={`100%`} gap={2} alignItems={`center`} justifyContent={`space-between`}>
            <Flex gap={2} alignItems={`center`}>
              <IconSet icon={link.icon} name={link.name} size={`24px`} />
              <Text>{link?.name}</Text>
            </Flex>
            <Center display={link?.analysis ? `flex` : `none`} w={`1.5rem`} h={`1.5rem`} fontSize={`0.8rem`} borderRadius={`100%`} color={`grey.100`} bg={`red.200`}>
              {link?.analysis || 10}
            </Center>
          </Flex>

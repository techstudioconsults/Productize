...
export const userApiSlice = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getAllAdminUsers: builder.mutation({
      query: (credentials) => {
        const url = `/users?role=ADMIN${credentials?.page ? `&page=${credentials.page}` : ''}${
          credentials?.startDate ? `&start_date=${credentials.startDate}` : ''
        }${credentials?.endDate ? `&end_date=${credentials.endDate}` : ''}${credentials?.status ? `&status=${credentials.status}` : ''}`;
    ...
    getAllPayoutHistory: builder.mutation({
      query: (credentials) => ({
        url: constructURL2(
          credentials,
          `/payouts?page=${credentials?.page}&start_date=${credentials?.startDate}&end_date=${credentials?.endDate}&status=${credentials?.status || ''}`,
          credentials?.status
        ),
        method: 'GET',
      }),

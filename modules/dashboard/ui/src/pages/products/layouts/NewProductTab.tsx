...
import {
  SetNewProductForm,
  ContentDeliveryForm,
} from '@productize-v1.0.0/modules/dashboard/feature';
...
export const NewProductTab = () => {
  ...
  const [updateProductStatus, updateProductStatusStatus] =
    ...
    if (state) {
      ...
      try {
        const res = await query(
          `post`,
          `/products/${state?.product?.id}?_method=PUT`,
          formData
        );
        ...
        if (res.status === 200) {
          ...
          toast({
            position: 'top',
            render: () => (
              <ToastFeedback message={`Product updated Successfully!`} />
            ),
          });
      ...
      try {
        ...
        if (res.status === 201) {
          ...
          toast({
            position: 'top',
            render: () => (
              <ToastFeedback message={`Product Created Successfully!`} />
            ),
          });
          navigate(`/dashboard/products/new#preview`, {
            state: { product: res.data },
  ...
  const handlePublishAction = async () => {
    const productID =
      hash === `#share` ? state?.product?.data?.id : state?.product?.id;
    try {
      ...
      if (res?.data?.status === `published`) {
        toast({
          position: 'top',
          render: () => (
            <ToastFeedback message={`Product	published Successfully!`} />
          ),
        });
        ...
        toast({
          position: 'top',
          render: () => (
            <ToastFeedback message={`Product	sent to draft Successfully!`} />
          ),
        });
        navigate(`/dashboard/products/new#preview`, {
          state: { product: res?.data },

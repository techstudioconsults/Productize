...
import { selectPaginationMetaData, useGetDraftProductsMutation } from '@productize/redux';
...
export const DraftTable = ({ draft, live, deleted, tableData }: tableProps) => {
    ...
    const handlePrevButton = async () => {
        try {
            await getDraftProducts({ link: paginate?.links?.prev });
        } catch (error) {
            console.log(error);
        }
    };
    const handleNextButton = async () => {
        try {
            await getDraftProducts({ link: paginate?.links?.next });
        } catch (error) {
            console.log(error);
        }
    };
